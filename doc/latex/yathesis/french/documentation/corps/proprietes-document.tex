...
\item[dans un fichier dédié]
  ...
  \zvref{sec-canevas-arborescence}.
...
titre\footnote{En outre, les titres et éventuels sous-titres figureront sur les
  pages de résumé (cf. \zvref{sec-abstract}) et de 4\ieme{} de couverture (cf.
  \zvref{sec-quatr-de-couv}).}.
%
...
\begin{dbremark}{Titre, sous-titre, champ disciplinaire et spécialité dans la
  ...
  correspondante \emph{dans la langue secondaire} de la thèse \aside*{par
    défaut l'anglais\footnote{Les langues principale et secondaire de la thèse
      sont détaillées \zvref{sec-langues}.}}.
...
\begin{docCommand}{submissiondate}{\marg{jour}\marg{mois}\marg{année}}
  ...
  \zvref{sec-versions}). Ses arguments sont soumis aux mêmes contraintes que ceux
  de la commande \refCom{date} (cf. \zvref{wa-format-date}).
\end{docCommand}
%
...
\begin{docCommand}[doc description=\mandatory]{doctoralschool}{\oarg{précision(s)}\marg{nom de l'école
  ...
  Cette commande définit l'école doctorale\footnote{Pour une \glsxtrshort{hdr},
    cet élément n'est pas \enquote{obligatoire} (cf. \zvref{sec-comm-oblig}).}.
\end{docCommand}
%
...
\begin{dbremark}{Laboratoires multiples}{}
  ...
  \item sur les pages de résumés et de 4\ieme{} de couverture
    (cf. \zvref{sec-abstract,sec-quatr-de-couv}), par défaut seul le laboratoire
  ...
  \item sur la page \aside{facultative} qui leur est dédiée
    (cf. \zvref{sec-laboratoires}), tous les laboratoires stipulés sont
    affichés.
  \end{itemize}
\end{dbremark}
...
\begin{docKey}{logoheight}{=\meta{dimension}}{pas de valeur par
  ...
  Par défaut, tous les logos ont une même hauteur de \qty{1.5}{\cm}
  mais la clé \refKey{logoheight} permet de spécifier une hauteur
  différente.
  \begin{dbexample}{Hauteur du logo d'institut}{}
    La commande de l'\zvref{ex-logoinst} aurait ainsi pu contenir :
\begin{preamblecode}[listing options={deletekeywords={[5]institute}}]
...
\begin{dbremark}{Instituts sous forme d'acronymes}{acronymes}
  ...
  \Package{glossaries-extra}. L'\zvref{acronymes} donne un aperçu de la procédure.
\end{dbremark}
...
\begin{docKey}[][doc updated=2016-10-30]{associateprofessor}{=\docValue{true}\textbar\docValue{false}}{par défaut
  ...
  \changes*{v0.99o}{2016-10-30}{\glsxtrshort{mcf} désormais traduit en anglais par
    ...
    plus par
    \enquote{Lecturer})\protect\footnote{Cf. \zvref{sec-expressions-cles} au cas
...
\begin{dbremark}{Corporation non prédéfinies}{}
  ...
  ci-dessus. La \zvref{sec-nouveaux-corps} explique comment procéder.
\end{dbremark}
%
...
\begin{docKey}{affiliation}{=\meta{institut}}{pas de valeur par défaut,
  ...
  \footnote{La \zvref{rq-acronymes} s'applique également ici : plutôt que
    spécifié tel quel, l'acronyme d'un \meta{institut} peut être géré par le
    \Package{glossaries-extra}.}  auquel est affilié un membre du jury.
\end{docKey}
...
à l'\zvref{wa-virgule}. De façon générale, il n'est pas indispensable de faire
...
\begin{dbwarning}{Préposition inadaptée}{}
  ...
  référera à la \zvref{sec-expr-separ-les}.
\end{dbwarning}
%
