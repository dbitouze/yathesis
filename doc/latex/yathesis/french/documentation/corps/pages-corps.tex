...
\begin{dbremark}{Scission du mémoire en fichiers parent et enfants}{}
  ...
  pour ce faire, standard, est rappelée \zvref{sec-repart-du-memo}.
\end{dbremark}
...
\zvref{sec-chap-non-numer,sec-intit-altern,sec-chapitres-numerotes}) :
\begin{description}
...
est précisée et synthétisée au \zvref{tab-commande-chapter-section}.
...
\begin{docCommand}[doc new=2016-12-08]{section}{\oarg{alt. en {\normalfont\ttfamily\glsxtrshort*{tdm}}}\oarg{alt. en entête}\marg{titre}}
  ...
  \item alternatif en entête est \meta{alt. en entête}.
  \end{itemize}
  % Son usage précis est synthétisé au \zvref{tab-commande-chapter-section}.
\end{docCommand}
%
\begin{table}[htb]
  \centering
...
\begin{dbremark}{Titres alternatifs différenciables aussi pour
  ...
  \zvref{tab-commande-chapter-section} : elles admettent donc elles aussi deux
...
\begin{dbexample}{Introduction}{}
  ...
  Le code suivant produit la \zvref{fig-introduction} illustrant une introduction
  (générale) non numérotée\footnote{Et, au passage, une table des matières
    locale. Plus de détails \zvref{sec-localtoc}.}. On constate que, bien que
...
le style |PetersLenny| du \Package{fncychap} (cf. \zvref{fig-chapitre}). La
\zvref{sec-style-des-tetes} explique comment ceci peut être modifié.
...
\begin{docCommand}[doc description=\mandatory]{printbibliography}{\oarg{options}}
  ...
  \zvref{fig-printbibliography}). Mais elle a été légèrement redéfinie de sorte
...
\begin{dbwarning}{Package \package{biblatex} non chargé par défaut}{}
  ...
  chapitres (cf. \zvref{sec-localbibs}).
\end{dbwarning}
