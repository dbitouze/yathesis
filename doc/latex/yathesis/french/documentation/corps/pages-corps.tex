...
\index{titre alternatif}

\changes{v0.99p}{2016-12-01}{Commandes \protect\refCom{chapter} et
  \protect\refCom{section} pourvue d'un argument optionnel supplémentaire
  permettant de stipuler un titre \meta{alternatif} en entête différent de celui
  en \gls{tdm}}%

Avec la \yatCl{}, les entêtes de la plupart des pages contiennent
le \meta{titre} du chapitre et le \meta{titre} de l'éventuelle section en cours
(cf. \vref{cha-pagination}).

Ce \meta{titre} est par défaut celui stipulé en argument obligatoire des
commandes \docAuxCommand{chapter} et \docAuxCommand{section} et figure alors
également dans le fil du texte et en \gls{tdm}\signet{}.
...
%
Habituellement, il est possible de faire figurer en \gls{tdm}\signet{} et en
entête \emph{un} titre \meta{alternatif} : il suffit pour cela de recourir
à l'\emph{unique} argument optionnel des commandes \docAuxCommand{chapter} et
\docAuxCommand{section}. Mais ce titre \meta{alternatif} est alors
nécessairement \emph{identique} en \gls{tdm}\signet{} et en entête.

La \yatCl{} fournit une fonctionnalité supplémentaire : grâce aux \emph{deux}
arguments optionnels des commandes \docAuxCommand{chapter} et
\docAuxCommand{section} qu'elle implémente, le titre \meta{alternatif} en entête
peut être différencié de celui en \gls{tdm}\signet{}.
...
\end{docCommand}
%
\begin{docCommand}[doc new=2016-12-01]{section}{\oarg{alt. en {\normalfont\ttfamily\acrshort*{tdm}}}\oarg{alt. en entête}\marg{titre}}
...
\end{table}
%
\begin{dbremark}{Titres différenciés alternatifs aussi pour les chapitres et
  ...
  arguments optionnels permettant de différencier les titres \meta{alternatifs}
...
correspondantes. Celles-ci ont toutefois été quelque peu modifiées afin d'en
simplifier l'usage.
...
\begin{dbremark}{Variantes étoilées des commandes de structuration modifiées}{}
  ...
  \begin{enumerate}
  \item automatiquement, le \meta{titre} (ou l'un de ses titres
    \meta{alternatifs}) correspondant figure :
    \begin{enumerate}
    \item en \gls{tdm} ;
    \item en entêtes (pour les chapitres et sections non numérotés
      seulement) ;
    \end{enumerate}
  \item les sous-unités qu'elles contiennent peuvent (et même \emph{doivent})
    être créées avec les versions \emph{non} étoilées des commandes
    correspondantes : elles seront néanmoins \emph{non} numérotées.

    Ainsi, si un chapitre est non numéroté, les sections, sous-sections et
    sous-sous-sections qu'il contient doivent elles aussi ne pas être
