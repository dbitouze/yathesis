...
\makeindex[name=commands,columns=2,title=Index des commandes,options=-M denisbdoc.xdy -L french,intoc]
\makeindex[title=Index des concepts,columns=3,options=-M denisbdoc.xdy -L french,intoc]
%
...
\AfterEndPreamble{%
  ...
  \changes{v0.99o}{2016-10-16}{Documentation révisée et augmentée d'un index
    des concepts}%
...
\AtEndDocument{%
  ...
  %
  \indexprologue{%
    Dans cet index, les types :
    \begin{itemize}
    \item commandes ;
    \item environnements ;
    \item clés ;
    \item valeurs de clés ;
    \end{itemize}
    des entrées sont indiqués par des couleurs (selon la convention précisée
    \vref{sec-comm-envir-cles}).%
  }%
  \printindex[commands]
  %
  % Some hack against point 1 of this issue: \url{https://github.com/ho-tex/oberdiek/issues/12}
