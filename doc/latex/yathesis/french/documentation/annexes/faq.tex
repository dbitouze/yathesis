...
En cas de problème d'utilisation\footnote{À ne pas confondre avec un bogue ou
  une fonctionnalité manquante, cf. \zvref{faq-bogues}.} :
\begin{enumerate}
...
\item Pour un dysfonctionnement\footnote{À ne pas confondre avec un
    ...
    cf. \zvref{faq-probleme-utilisation}.}  ou une amélioration :
  \begin{enumerate}
  \item avant de le signaler ou de la suggérer, s'assurer qu'ils n'ont pas déjà
    ...
    \item en lisant l'\zvref{cha-incomp-conn} ;
...
Ça peut être grave. Cf. \zvref{rq-packages-anciens} pour plus de précisions.
...
automatiquement (cf. l'\zvref{sec-packages-charges-par} pour la liste des
packages automatiquement chargés). Supprimer cette commande devrait résoudre le
problème (cf. également l'\zvref{wa-packages-a-ne-pas-charger}).
...
\item soit en y resaisissant manuellement toutes les caractéristiques
  nécessaires définies au \zvref{cha-caract-du-docum} ;
...
ce soit le cas, en \emph{ne} recourant \emph{pas} à l'option \docAuxKey*{toc} du
...
\begin{enumerate}
\item les caractéristiques du document (cf. \zvref{cha-caract-du-docum}), il
  suffit de les saisir\footnote{Cf. \zvref{sec-lieu-de-saisie}.} :
  \begin{itemize}
  \item soit dans le \emph{corps} du fichier (parent) de la thèse\footnote{Mais
      cf. alors \zvref{wa-avant-maketitle}.} (et donc \emph{pas} dans son
    \emph{préambule}) ;
...
\refKey{secnumdepth}. Cf. \zvref{sec-profondeur-de-la} pour plus de précisions.
