        ...
        \item Sur certaines pages, notamment celles de titre :
          ...
          \item\label{item-expression} les expressions fournies par la
            ...
            \item des hyperliens vers le fichier de configuration de la thèse
              (cf.  {\NoAutoSpacing\vref{rq:configurationfile}}) où il est possible de les
...
\begin{docCommand}{submissiondate}{\marg{jour}\marg{mois}\marg{année}}
  ...
  (cf. {\NoAutoSpacing\vref{wa:format-date}}).
\end{docCommand}
...
\begin{dbwarning}{Packages automatiquement chargés à ne pas charger
  ...
  (tel que signalé à la {\NoAutoSpacing\vref{faq:option-clash}}).
\end{dbwarning}
...
\begin{docKey}{setspace}{=\marg{option(s)}}{pas valeur par défaut,
  ...
  (cf. {\NoAutoSpacing\vref{wa:space-setspace}}).
\end{docKey}
...
sont listées\footnote{Et classées par ordre alphabétique des expressions \meta{en
    français}.} dans le \vref{tab-expressions-cles} et y sont identifiées par un
\meta{label} permettant de les redéfinir (voire de les définir, cf.
{\NoAutoSpacing\vref{ex:doctor}}) au moyen de la commande \refCom{expression}
suivante.
%
