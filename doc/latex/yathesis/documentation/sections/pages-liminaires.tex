...
\begin{docCommand}{frontepigraph}{\oarg{langue}\marg{épigraphe}\marg{auteur}}
  ...
  spécifier en argument optionnel% {\NoAutoSpacing
    \footnote{Si cette \meta{langue} est autre que
    le français ou l'anglais, elle doit être explicitement chargée en option de
    la commande \docAuxCommand{documentclass} (cf.
    % remarque \ref{rq:languessupplementaires}%
    \vref{rq\string:languessupplementaires}%
    ).}% }
  .
\end{docCommand}
...
(cf. {\NoAutoSpacing\vref{wa:frenchabstract}}), une préface, un avant-propos, etc.
