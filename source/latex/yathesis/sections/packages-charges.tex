\chapter{Packages (non) chargés par la \yatcl{}}\label{cha:packages-charges}
...
\item[\package{draftwatermark} :] texte en filigrane\footnote{Chargé
    seulement si l'une ou l'autre des options de classe
    \refKey{draft} ou \refKey{intermediate} est utilisée.} ;
...
\item[\package*{idxlayout} :] correction d'un bogue affichant trop haut
  ...
  \docAuxKey{columns=1}\footnote{Il suffit de le spécifier au moyen de
    \protect\lstinline+\\idxlayout\{columns=+\meta{n}\protect\lstinline+\}+ si
    on souhaite un index, non pas sur $1$, mais sur \meta{n} colonnes.} ;
...
\item[\package{hypcap} :] liens hypertextes pointant au début des
  flottants\ifnoprinted. Option par défaut : \docAuxKey{all} ;
\item[\package{bookmark} :] gestion des signets\ifnoprinted. Option par défaut
  : \docAuxKey{numbered}.
\end{description}
...
\item[\package*{cleveref} :] puissante gestion des références croisées.
\end{description}
