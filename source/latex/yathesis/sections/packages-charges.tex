...
\item[\package*{index} :] gestion du ou des index\footnote{Pour la gestion
    ...
    du \Package{index} par la classe est nécessaire pour des raisons
    techniques.} ;
\item[\package*{idxlayout} :] correction d'un bogue affichant trop haut
  l'intitulé \enquote{Index} de l'index ;
  \begin{description}
  \item[option par défaut :] \docValue*{columns=1} ;
  \item[personnalisation :] commande propre \docAuxCommand*{idxlayout} ;
  \end{description}
  % ^^A \footnote{Si on souhaite un index non pas sur $1$ mais
  % ^^A sur \meta{n} colonnes, il suffit de le spécifier au moyen de
  % ^^A\protect\lstinline+\\idxlayout\{columns=+\meta{n}\protect\lstinline+\}+.} ;
...
\item[\package*{varioref} :] références croisées améliorées ;
  \begin{description}
  \item[personnalisation :] option de la \yatcl \refKey{varioref} ;
  \end{description}
