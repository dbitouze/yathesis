...
\begin{dbremark}{Fichier de données}{titlefile}
  ...
  \item soit dans un fichier à nommer \file{\titlefile} et à placer dans
    ...
    et sous-dossier \phrase{tous deux prévus à cet effet} sont à créer au
...
\begin{dbwarning}{Fichier de données à ne pas importer}{}
  ...
  importé \phrase*{au moyen d'une commande \docAuxCommand{input} ou assimilée}.
\end{dbwarning}
...
\end{docCommand}
%
\begin{docCommand}[doc description=\mandatory]{laboratory}{\oarg{précision(s)}\marg{nom du
      laboratoire}\marg{adresse du laboratoire}}
...
% ^^A   \begin{bodycode}[title=Préparation du titre (p. ex. dans le \File{\titlefile})]
...
La commande \refCom{maketitle} suivante \emph{produit}, à partir des données
...
%^^A La ou les pages produites par la commande \refCom{maketitle} contiennent
%^^A automatiquement les données de la thèse stipulées au moyen des commandes
%^^A décrites \vref{sec:preparation-titre}.
