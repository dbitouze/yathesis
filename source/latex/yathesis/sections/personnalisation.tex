...
\item dont la personnalisation se fait habituellement par le biais d'options
  ...
  seulement par ce biais-là).%^^A
  % ^^A\footnote{En particulier, ne sont pas concernés les
  % ^^Apackages dont les options peuvent être passées indifféremment en argument
  % ^^Aoptionnel de \protect\docAuxCommand{usepackage} ou au moyen d'une
  % ^^Acommande de configuration propre ; il en est ainsi du \Package{geometry}
  % ^^Aqui dispose de la commande \protect\docAuxCommand*{geometry}.}.
\end{itemize}
