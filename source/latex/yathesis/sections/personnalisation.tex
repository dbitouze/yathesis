...
\begin{docKey}{version}{=\docValue{inprogress}\textbar\docValue{inprogress*}\textbar\docValue{submitted}\textbar\docValue{submitted*}\textbar\docValue{final}\textbar\docValue{draft}}{pas
    ...
    \item[\docValue{submitted}.] Cette valeur produit une version du document
      ...
      \item
        %
        \changes{v0.99f}{2014/07/11}{Modification de la mise en page des pages
          de titre : les date de soutenance et composition du jury absentes (et
          non obligatoires) en versions \enquote{à soumettre}}
        %
...
\changes{v0.99g}{2014/07/13}{La clé \protect\refAux{chap-style} est remplacée
  par (et devient un alias de) la clé \protect\refKey{fncychap}}%
...
package\footnote{Par souci de compatibilité ascendante, la clé désormais
  obsolète \docAuxKey{chap-style} est un alias de la clé \refKey{fncychap}.}.
%
{%
  \tcbset{before lower=\vspace*{\baselineskip}\par}
...
\label{sec:hdr}
%
\changes{v0.99f}{2014/07/11}{La classe peut désormais être utilisée pour les
  \gls{hdr}}
...
n'est pas possible. Pour pallier à cela, \yat{} fournit des options de classe
...
pour savoir quelles valeurs peuvent être passées à ces options).
...
\begin{docKey}{datatool}{=\marg{option(s)}}{pas valeur par défaut,
  ...
  \Package{datatool}.
\end{docKey}
\begin{docKey}{titleps}{=\marg{option(s)}}{pas valeur par défaut,
    initialement vide}
  Cette option permet de passer une ou plusieurs \meta{option(s)} au
  \Package{titleps}.
\end{docKey}
...
\begin{docKey}{datetime}{=\marg{option(s)}}{pas valeur par défaut,
  ...
  \Package{datetime}.
\end{docKey}
\begin{docKey}{varioref}{=\marg{option(s)}}{pas valeur par défaut,
    initialement vide}
  Cette option permet de passer une ou plusieurs \meta{option(s)} au
  \Package{varioref}.
\end{docKey}
...
\changes{v0.99f}{2014/07/11}{Possibilité de spécifier et définir de nouveaux
  rôles pour les personnes}
