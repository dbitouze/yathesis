...
\begin{enumerate}
\item Proposer une clé \docAuxKey{frametitle} prenant les valeurs
  \docValue*{single}, \docValue*{none}, \docValue*{leftline},
  \docValue*{topline}, \docValue*{bottomline}, \docValue*{lines},
  \docValue*{single}, \docValue*{shadowbox} (cf. \Package{listings}) permettant
  de spécifier l'encadrement du titre (en cas de
  \docAuxKey{frametitle}×=×\docValue*{none}, le pas afficher l'expression
  \enquote{Titre de la thèse}).
\item Dans la commande (privée) ×\YAD@alaudeldu×, le \enquote{select case}
  devrait être macro à part entière réutilisable ailleurs, notamment pour le
  \enquote{L'} de la clause de non-resonsabilité.
\item Fournir les (dossiers des) spécimens et canevas (seulement ?) sous la
  forme de fichiers \file{.zip} vers lesquels des hyperliens de la
  documentation puissent pointer (plus simple à manipuler et moins bogogène que
  pointant vers des dossiers). Fournir également des hyperliens vers des
  versions en line de ces \file{.zip}.
\end{enumerate}
...
\item Documenter le canevas \enquote{à plat}.
\item Fournir et documenter le canevas \enquote{en relief}.
\item Recourir au \Package{changes} pour indiquer les changements d'une version
  à l'autre de la classe.
  % ^^A \item Revoir les instructions d'installation de la classe et de
  % ^^A production de sa documentation.
  % ^^A \item Réduire la profondeur de la table des matières.
\item Prévoir une version imprimée.
  % ^^A \item Insérer un graphique du \Package{pgfplots} dans le
  % ^^A \foreignquote{english}{sample}.
\end{enumerate}
