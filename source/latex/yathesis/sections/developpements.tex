...
\item Dans les commandes ×\RequirePackage×, indiquer les \enquote{releases}
  minimum.
\item Ne pas fournir les fichiers \file{fonte-pdflatex.tex} et
  \file{fonte-xelatex.tex}.
...
\item Documenter les dossiers et fichiers connus de la \yatcl{} :
  ...
  \item etc.
  \end{itemize}
  % ^^A \item Est-il opportun de prévoir des fichiers automatiquement chargés
  % ^^A par la \yatcl{}, par exemple :
  % ^^A \begin{itemize}
  %   ^^A \item \file{\acronymsfile} ;
  %   ^^A \item \file{macros.tex} ;
  %   ^^A \item etc.
  %   ^^A \end{itemize}
  % ^^A qui permettrait de ne pas avoir à les charger manuellement au moyen de
  % ^^A \docAuxCommand{input} ?
